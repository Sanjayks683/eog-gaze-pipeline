...
predictions: data/processed_nested/<dataset>/ (git-ignored).
"""
...
def main() -> None:
    ...
    selected_pred = {crit: np.full_like(y, np.nan) for crit in ("fixation", "rmse")}
    ...
    for k, (tr, te) in enumerate(outer):
        ...
        for i, candidate in enumerate(candidates):
            ...
            print(f"fold {k} [{i + 1}/{len(candidates)}] {describe(candidate)}: inner fixation "
                  f"{inner_scores[candidate]['score_fixation']:.3f} rmse {inner_scores[candidate]['score_rmse']:.3f} "
                  f"({time.time() - t_start:.0f}s)", flush=True)
        best = {crit: min(candidates, key=lambda c: inner_scores[c][f"score_{crit}"]) for crit in selected_pred}
        for crit, candidate in best.items():
            selected_pred[crit][te] = fixed_pred[candidate][te]
        folds_report.append({
            "fold": k,
            "test_subjects": sorted(set(subjects[te].tolist())),
            "selected": {crit: describe(c) for crit, c in best.items()},
            "inner_scores": {describe(c): {key: s[key] for key in ("score_fixation", "score_rmse",
                                                                   "fixation_mae_h_deg", "fixation_mae_v_deg",
                                                                   "rmse_h_deg", "rmse_v_deg")}
                             for c, s in inner_scores.items()},
        })
        print(f"fold {k} selected: {folds_report[-1]['selected']}", flush=True)

    fixed = {describe(c): evaluate(y, p, subjects, fixation) for c, p in fixed_pred.items()}
    selected = {crit: evaluate(y, p, subjects, fixation) for crit, p in selected_pred.items()}
    best_fixed = {crit: min(fixed, key=lambda name: fixed[name][f"score_{crit}"]) for crit in selected}
    results = {
        ...
        "best_fixed_on_outer_folds": {crit: {"candidate": name, **fixed[name]} for crit, name in best_fixed.items()},
        "fixed_candidates": fixed,
        "folds": folds_report,
        "runtime_sec": round(time.time() - t_start),
    }
    ...
    with open(os.path.join(out_dir, "fixed_candidates.csv"), "w", newline="", encoding="utf-8") as f:
        ...
        for name, r in sorted(fixed.items(), key=lambda kv: kv[1]["score_fixation"]):
            writer.writerow([name, r["fixation_mae_h_deg"], r["fixation_mae_v_deg"], r["rmse_h_deg"], r["rmse_v_deg"]])
